 ...
 *
 */
export function getEndowmentsForConfig(
  sourceRef,
  config,
  unwrapTo,
  unwrapFrom,
...
function applyEndowmentPropDescTransforms(
  ...
  newPropDesc = applyGetSetPropDescTransforms(
    newPropDesc,
    unwrapFromCompartment.globalThis,
    unwrapToGlobalThis,
  );
  return newPropDesc;
}
...
function applyFunctionPropDescTransform(
  ...
  const unwrapTest = thisValue => {
    ...
    return (
      thisValue === unwrapFromCompartment.globalThis ||
      unwrapFromCompartment.__isKnownScopeProxy__(thisValue)
    );
  };
